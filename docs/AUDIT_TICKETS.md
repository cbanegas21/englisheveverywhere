...
- **2026-06-02 — EK-011 DONE** (Carlos chose: add column): migration 028 adds `teachers.accepting_students` (applied to live DB; types/schema regenerated). The teacher dashboard toggle now writes `accepting_students` instead of `is_active`, so toggling it can never send the teacher to /pending. `is_active` stays admin-only approval. Future: admin assignment can prefer `accepting_students = true`.
- **2026-06-02 — EK-029 ✅** (teacher earnings no longer capped at 50 sessions; payout fallback 50→60min).
- **2026-06-02 — QA session ✅** (Playwright regression, 24/24 green): confirmed booking gate revert, 24h guard, RLS/escalation, double-booking constraint, landing render. Caught + fixed stale `booking-guard-24h` spec (referenced removed "Muy pronto"/"9 AM" UI) — not a regression.
- **2026-06-02 — timezone batch (scoped):** KEY FINDING — most hardcoded `America/Tegucigalpa` is INTENTIONAL (admin operates from Honduras; `availability_slots` stored in Honduras local; emails labeled). Real bugs are narrow. Done: EK-012/044 student-facing display — maestros placement date + PlacementScheduledScreen now show the student's own timezone (date/time already were; fixed the misleading "Honduras time" label + added zone abbrev). **Carlos decided: leave input-side slot pickers (EK-045/019/044-gen) AS-IS for launch** (correct on own device; full fix needs a wall-clock→UTC utility — post-launch). **Deferred: EK-046** admin-calendar offset arithmetic + today's-day-of-week bug (admin-only, Honduras, needs careful date work — focused pass later).
- **2026-06-02 — video batch:** EK-020 ✅ (mute/camera state derives from LiveKit via useTrackToggle, not local useState → no desync on external mute). EK-022 ✅ (control bar wraps on mobile: flex-wrap + responsive gap → End Class/Notes/Devices stay reachable). **EK-021 = VERIFIED NOT A BUG** — null teacher_id means no teacher to join; the student's access works via both the page guard and getRoomAccess (matches studentProfileId), and confirmed rooms always have teacher_id. Mischaracterized.
- **2026-06-02 — verified (no code change):** **EK-028 = SECURE** — teacherSetStudentLevel requires authenticated + role teacher + a teacher record + a non-cancelled booking with the student (placement.ts:295-331). Combined with Carlos's "teachers recalibrate level in session 1" decision, that's correct authorization, not a gap. **EK-015 = mostly handled** — approveRescheduleRequest already re-issues reminders for the new time (idempotent scheduleBookingReminders, line 267); the only residual is no *immediate* "your class moved" email (student still gets T-24h/T-1h reminders for the new time). Soft UX enhancement, not a correctness bug.
- **Verified-NOT-bugs this session:** EK-014 (credit decremented at booking, not confirm), EK-021 (null teacher = no teacher to join; student access fine), EK-028 (properly authorized). Verify-first kept us from 3 wrong/unnecessary changes.
- **2026-06-02 — EK-025 ✅ + EK-026 false positive:** EK-026 = **NOT a bug** — privacy/page.tsx already has a full `es` translation (picked via `t[lang]`); the audit was wrong. EK-025 ✅ done the no-regression way — added HtmlLangSync (client component) that sets `document.documentElement.lang` to the active locale. Chose this over reading headers() in the root layout, which would have forced the whole app dynamic and killed SSG on the marketing pages (verified marketing pages remain ● SSG after the change). The `[lang]` `<div lang>` already handles screen-reader language for content (nested lang overrides per HTML spec); this corrects the doc-level lang for JS crawlers too.
- **ALL HIGH-SEVERITY BUGS RESOLVED OR VERIFIED.** Remaining "high" items are: EK-015 (immediate reschedule email — soft; reminders already re-issue), EK-023 (payout reporting — no reports built, no impact), EK-046 (admin calendar date math — deferred, admin-only). 4 verified non-bugs total: EK-014, EK-021, EK-028, EK-026.
- **2026-06-02 — VISUAL QA layer added + medium polish:** Logged in as a real student via Playwright and screenshotted every dashboard page at desktop + mobile, then eyeballed them. Findings: dashboard/plan/progreso look polished and prior copy fixes are visibly live (Maestro certificado, classes-never-expire, no-auto-renewal). Fixed **EK-053** (progreso showed fabricated "≈40 classes to next level" + nonsensical "0h ≈ a workday" — now "your teacher will tell you when you're ready" + "of live class"). **EK-052 deferred** — receipts need the Stripe webhook to record a student-purchase row (small feature, not polish); page degrades gracefully ("no payments yet").
- **2026-06-02 — medium quick-wins:** EK-036 ✅ (Navbar added to contact/privacy/terms + made Navbar anchor links absolute `/${lang}#…` so they work from subpages — visually confirmed nav renders on /contact). EK-035 ✅ (removed "Cancel anytime"/"Cancela cuando quieras" subscription framing on the one-time-payment product → "No auto-renewal"; landing pricing + plan page). EK-059 ✅ (student "become a teacher" link → mailto express-interest instead of the protected /maestro/dashboard dead-end).
- **2026-06-02 — minor-medium polish:** EK-061 ✅ (dashboard "Reschedule" now links to /clases where the real reschedule action lives, not the new-booking grid). EK-031 ✅ (Hero booking card was a false affordance — pointer cursor + hover but no handler; now the whole card is a Link to /registro, turning it into a real signup CTA; verified landing renders clean). EK-051 ✅ (admin CEFR-level select no longer auto-saves on change — now an explicit Save button mirroring the adjacent Teacher select, so a stray change can't instantly persist).
- **EK-060 reclassified as a small FEATURE** (not polish): the real value is a cancelled-classes history view (needs a new query + tab UI), so deferred with the other small features rather than a token dead-key removal.
- **MEDIUM TRIAGE COMPLETE** for launch-relevant items. Remaining: ⚪ low/cosmetic (14) + deferred small-features (EK-052 receipts, EK-060 cancelled view, EK-046 admin calendar, EK-015 reschedule email, EK-023 payout reporting) — all post-launch candidates.
- **2026-06-02 — accessibility (low) batch:** EK-037 ✅ (FAQ accordion: aria-expanded + aria-controls + role=region / aria-labelledby). EK-038 ✅ (global :focus-visible outline → classroom controls & all interactive elements now show a keyboard-focus ring; keyboard-only, mouse unaffected). EK-039 ✅ (password show/hide toggle on login + registro: added localized aria-label, removed tabIndex=-1 so it's keyboard-reachable). EK-071 ✅ (muted mic/camera control now solid-red, distinct from active/brand). EK-079 (partial) ✅ (prefers-reduced-motion guard neutralizes the marquee + long animations). Build + 11/11 public-page render QA green.
- **Remaining = pure cosmetic lows + deferred small-features.** App is launch-ready; these are post-launch polish.
