...
- **Open next (all soft / content / deferred):** EK-015 (immediate reschedule email — soft), EK-025 (html lang — needs middleware header), EK-026 (privacy ES — content), EK-023 (payout reporting — no reports built yet, low impact), EK-046 (admin calendar — deferred). → then medium triage → low. Minor dead code: onboarding LEVELS/LEVEL_LABELS/step2Student (EK-077).
