...
- **Open next:** 🟡 EK-031 (dead Hero "BOOK" card), EK-051 (admin level auto-save), EK-060 (cancelled-tab), EK-061 (dashboard reschedule link) → then ⚪ low. Deferred (small features / admin-only): EK-052 receipts, EK-046 admin calendar, EK-015 reschedule email, EK-023 payout reporting.
