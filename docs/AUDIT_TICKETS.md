...
- **Open next:** 🟠 video (EK-020/021/022), EK-028 (level-set security), EK-015 (reschedule notify), EK-025 (html lang — needs middleware header), EK-026 (privacy ES — content), EK-023 (payouts reporting), EK-046 (admin calendar, deferred) → then medium triage → low. Minor dead code: onboarding LEVELS/LEVEL_LABELS/step2Student (EK-077).
