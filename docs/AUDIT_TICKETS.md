...
- **Open next:** 🟡 MEDIUM TRIAGE — EK-036 (navbar on legal/contact pages), EK-035 (subscription-framing copy "cancel anytime"/"renews on"), EK-031 (dead Hero "BOOK" card), EK-051 (admin level auto-save), EK-060 (cancelled-tab), EK-059 (student "become a teacher" dead link) → then ⚪ low. Deferred: EK-052 (receipts feature), EK-046 (admin calendar), EK-015 (reschedule email), EK-023 (payout reporting).
