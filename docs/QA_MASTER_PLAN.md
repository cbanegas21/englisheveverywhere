...
- **OPEN (flagged, NOT fixed — product decisions for Carlos):** **SB-01** a deactivated teacher's cleared earnings are stranded (sweep + preview filter `is_active=true`; mitigated user-facing by the pending-gate, but money owed is never swept) → pay them on reactivation vs freeze? **SB-02** `toggleTeacherActive` deactivates WITHOUT cancelling/re-queuing the teacher's live student bookings (unlike reject/delete) → the student keeps a confirmed class the teacher can't teach (confirm/decline `is_active`-blocked) and it doesn't re-enter the admin unassigned queue (teacher_id still set) → **recommend re-queue (teacher_id=null, status=pending) on deactivate**, Carlos picks re-queue vs cancel+refund. **SB-03** a student can report a "teacher no-show" + get a refund on a booking that never had a teacher assigned (no `teacher_id` guard) — net-zero credits, part of the existing CRM-10 unilateral-no-show-claim policy. **SB-09** assigning a teacher to a FREE placement silently locks them as the student's `primary_teacher_id` (no `type='class'` guard) → switching to a different paid-class teacher later needs `force=true`. **SB-10** ganancias "Sessions" counts a no-show (completed, $0) while teacher-home `total_sessions` doesn't (3 vs 2) — earnings stay correct, only the count label diverges; + the student's "Clases completadas"/Hours likewise count a no-showed class.
