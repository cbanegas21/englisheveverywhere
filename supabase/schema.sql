...
alter table public.auth_attempts add constraint auth_attempts_action_check CHECK ((action = ANY (ARRAY['login'::text, 'signup'::text, 'reset'::text, 'createBooking'::text, 'bookPlacementCall'::text, 'requestEmailChange'::text])));
